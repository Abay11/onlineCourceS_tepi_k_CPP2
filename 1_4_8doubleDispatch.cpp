...
Мультиметод — это обобщение понятия виртуального метода. При вызове виртуального метода
выбор конкретной реализации зависит от типа одного объекта
...
Рассмотрим следующий пример. У классаShape (фигура) есть три наследника:
...
/*Однако такой код нельзя назвать хорошим: реализация этой функции будет состоять из 9 веток if-else,
