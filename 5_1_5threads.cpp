...
auto map_reduce(It p, It q, F1 f1, F2 f2, size_t threads = 1) -> decltype(f2(f1(*p),f1(*p))){
    using return_value = decltype(f2(f1(*p),f1(*p)));
    int dist = (std::distance(p, q) / threads);
    It begin, end = p;
    std::vector<return_value>values;
    std::vector<std::thread> threads_;
    return_value res;
    for (int i = 0; i < threads; ++i) {
        begin = end;
        if(i == threads - 1) end = q;
        else std::advance(end, dist);
        threads_.push_back(std::thread([&res, f2, f1, &begin, end](){
            res = f1(*begin);
            while(++begin != end){ res = f2(res, f1(*begin));
            }
        }));
